Islamic Reminder
This is a small project that shows a random Islamic reminder.  

## What is this project?
It is a simple web page that gives one reminder when you open it.  
You can press a button to get a new reminder.  
There is also a button to copy the reminder.

## How to use it?
1. Open the file `index.html` in your browser.
2. You will see a reminder.
3. Press the button to change the reminder.
4. Press the copy button if you want to copy the text.

## Why I made it?
I made this project to practice HTML, CSS, and JavaScript.  
I also wanted something simple that helps poeple remember Allah.

